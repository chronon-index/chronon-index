...
- [ ] B-uc2-19 | deps: D-01 | refs: ruling B-uc2-02(c), RP#M2 | Model-layer integration of the keyless reference: graduation (Kannisto), life_table (AK a0), lee_carter as tly modules with tests; numpy/pandas permitted in the MODEL layer only — float quarantine still guards every published path; NCHS published tables (not WONDER — 403s from cloud IPs) if US single-age annual is needed
