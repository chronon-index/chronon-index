...
- [ ] B-uc2-03a | deps: B-uc2-01 | refs: SPEC#2, RP#II-D3, SPEC#0 G6 | World Mortality Dataset (Karlinsky & Kobak, GitHub raw — keyless): verify license honestly, snapshot weekly/monthly deaths CSV, parser → Decimal; manifest rows; unit tests. Serves EITHER B-uc2-02 ruling (as automated feed, or as triangulation)
...
- [ ] B-uc2-04 | deps: B-uc2-03a | refs: SPEC#2, RP#II-D6, RP#X-P7 | Versioned baseline expected-deaths method + per-country coverage metadata (measured vs imputed); tests (works on WMD feed; STMF joins when B-uc2-03 unblocks)
